...
class PDBArray(np.ndarray):
    def __new__(cls, pdb_string):
        ...
        if os.path.isfile(pdb_string): pdb_string = open(pdb_string, 'r').read()
        if len(pdb_string) == 4:  
        ...
        universe = mda.Universe(StringIO(pdb_string), format="PDB")
        coords = universe.atoms.positions
        obj = coords.view(cls)
        obj.universe = universe
        obj.original_pdb = pdb_string
        return obj
    
    def __array_finalize__(self, obj):
        if obj is None: return
        self.universe = getattr(obj, 'universe', None)
        self.original_pdb = getattr(obj, 'original_pdb', None)
